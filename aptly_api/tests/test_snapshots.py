...
class SnapshotAPISectionTests(TestCase):
    ...
    def test_create(self, *, rmock: requests_mock.Mocker) -> None:
        rmock.post(
            "http://test/api/repos/aptly-repo/snapshots",
    ...
    def test_list_packages_details(self, *, rmock: requests_mock.Mocker) -> None:
        ...
        parsed = self.sapi.list_packages(
            "aptly-repo-1", query="Name (% postgresql-9.6.-postgis-sc*)", detailed=True, with_deps=True
        )[0]
        expected = Package(
            key="Pall postgresql-9.6-postgis-scripts 2.3.2+dfsg-1~exp2.pgdg90+1 5f70af798690300d",
    ...
    def test_diff(self, *, rmock: requests_mock.Mocker) -> None:
        rmock.get(
            "http://test/api/snapshots/aptly-repo-1/diff/aptly-repo-2",
    ...
    def test_create_from_mirror(self, *, rmock: requests_mock.Mocker) -> None:
        ...
        rmock.post(
            "http://test/api/mirrors/aptly-mirror/snapshots",
