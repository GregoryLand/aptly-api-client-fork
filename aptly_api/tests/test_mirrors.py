...
class MirrorsAPISectionTests(TestCase):
    ...
    def test_create(self, *, rmock: requests_mock.Mocker) -> None:
        ...
        assert len(rmock.request_history[0].json()) == len(expected)
    ...
    def test_create_all_args(self, *, rmock: requests_mock.Mocker) -> None:
        ...
        assert len(signature(self.miapi.create).parameters) == len(expected)
    ...
    def test_mirror_from_response(self, *, rmock: requests_mock.Mocker) -> None:  # noqa: ARG002
        resp = self.miapi.mirror_from_response(
            json.loads("""{
    ...
    def test_list(self, *, rmock: requests_mock.Mocker) -> None:
        ...
        assert self.miapi.list() == confirm

    def test_show(self, *, rmock: requests_mock.Mocker) -> None:
        rmock.get(
            "http://test/api/mirrors/aptly-mirror",
            text="""{"UUID": "2cb5985a-a23f-4a1f-8eb6-d5409193b4eb",
                  "Name": "aptly-mirror",
                  "ArchiveRoot": "https://deb.nodesource.com/node_10.x/",
                  "Distribution": "bionic", "Components": ["main"],
        ...
        assert self.miapi.show(name="aptly-mirror") == confirm

    def test_list_packages(self, *, rmock: requests_mock.Mocker) -> None:
        rmock.get(
            "http://test/api/mirrors/aptly-mirror/packages",
        ...
        assert self.miapi.list_packages(name="aptly-mirror", query=("nodejs"), with_deps=True) == confirm
