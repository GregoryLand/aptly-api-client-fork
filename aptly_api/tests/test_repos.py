...
class ReposAPISectionTests(TestCase):
    ...
    def test_create(self, *, rmock: requests_mock.Mocker) -> None:
        ...
        test = self.rapi.create("aptly-repo", comment="test", default_component="test", default_distribution="test")
        assert test == test_data

    def test_show(self, *, rmock: requests_mock.Mocker) -> None:
        rmock.get(
            "http://test/api/repos/aptly-repo",
        ...
        test = self.rapi.show("aptly-repo")
        assert test == test_data

    def test_search(self, *, rmock: requests_mock.Mocker) -> None:
        rmock.get(
            "http://test/api/repos/aptly-repo/packages", text='["Pamd64 authserver 0.1.14~dev0-1 1cc572a93625a9c9"]'
        )
        test_data = [
            Package(
        ...
        test = self.rapi.search_packages("aptly-repo")
        assert test == test_data
    ...
    def test_search_details(self, *, rmock: requests_mock.Mocker) -> None:
        ...
        test = self.rapi.search_packages("aptly-repo", detailed=True, with_deps=True, query="Name (authserver)")
        assert test == test_data
    ...
    def test_repo_edit(self, *, rmock: requests_mock.Mocker) -> None:
        ...
        test = self.rapi.edit("aptly-repo", comment="comment", default_distribution="stretch", default_component="main")
        test_data = Repo(name="aptly-repo", comment="comment", default_distribution="stretch", default_component="main")
        assert test == test_data
    ...
    def test_add_file(self, *, rmock: requests_mock.Mocker) -> None:
        rmock.post(
            "http://test/api/repos/aptly-repo/file/test/dirmngr_2.1.18-6_amd64.deb",
            text='{"FailedFiles":[],"Report":{"Warnings":[],' '"Added":["dirmngr_2.1.18-6_amd64 added"],"Removed":[]}}',
        )
        test_data = FileReport(
            failed_files=[], report={"Added": ["dirmngr_2.1.18-6_amd64 added"], "Removed": [], "Warnings": []}
        )
        test = self.rapi.add_uploaded_file("aptly-repo", "test", "dirmngr_2.1.18-6_amd64.deb", force_replace=True)
        assert test == test_data

    def test_add_dir(self, *, rmock: requests_mock.Mocker) -> None:
        rmock.post(
            "http://test/api/repos/aptly-repo/file/test",
            text='{"FailedFiles":[],"Report":{"Warnings":[],' '"Added":["dirmngr_2.1.18-6_amd64 added"],"Removed":[]}}',
        )
        test_data = FileReport(
            failed_files=[], report={"Added": ["dirmngr_2.1.18-6_amd64 added"], "Removed": [], "Warnings": []}
        )
        test = self.rapi.add_uploaded_file("aptly-repo", "test", force_replace=True)
        assert test == test_data
    ...
    def test_add_package(self, *, rmock: requests_mock.Mocker) -> None:
        ...
        test_data = Repo(name="aptly-repo", comment="", default_distribution="", default_component="")
        test = self.rapi.add_packages_by_key("aptly-repo", "Pamd64 dirmngr 2.1.18-6 4c7412c5f0d7b30a")
        assert test == test_data
    ...
    def test_delete_package(self, *, rmock: requests_mock.Mocker) -> None:
        ...
        test_data = Repo(name="aptly-repo", comment="", default_distribution="", default_component="")
        test = self.rapi.delete_packages_by_key("aptly-repo", "Pamd64 dirmngr 2.1.18-6 4c7412c5f0d7b30a")
        assert test == test_data
